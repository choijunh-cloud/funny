...
PDF = ROOT / "reports" / "2026-08-28-quick-comment-brief.pdf"
OUT = ROOT / "reports" / "pdf-pages"
HTML = ROOT / "reports" / "2026-08-28-quick-comment-brief-pages.html"
...
def _write_html(pages: list[Path]) -> None:
    ...
    for i, p in enumerate(pages, 1):
        ...
        if i == len(pages) and len(pages) > len(TITLES):
            title = "원문 부록"
        articles.append(
            f'    <article class="page" id="p{i}">'
            f"<h2>{i} / {len(pages)} · {title}</h2>"
            f'<img src="pdf-pages/{p.name}" alt="{i}쪽 {title}" /></article>'
        )
    HTML.write_text(
        f"""<!DOCTYPE html>
<html lang="ko">
<head>
  ...
  <title>8월 28일 Quick 코멘트 · PDF 페이지</title>
  <style>
...
<body>
  <div class="top">
    <div class="top-inner">
      <div class="brand">QUICK COMMENT BRIEF · PDF 페이지 · <b>8월 28일</b></div>
      <nav>
        {nav}
        <a href="2026-08-28-quick-comment-brief.html">HTML</a>
      </nav>
    </div>
  </div>
  <div class="wrap">
    <h1>A4 {len(pages)}쪽 · 여기서 바로 스크롤</h1>
    <p class="muted">PDF 앱이 없어도 됩니다. 위 번호로 이동. Quick 코멘트와 첨부 PDF 5건 전부 수록. 매수·매도 추천 아님.</p>

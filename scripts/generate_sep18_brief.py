...
class Notes:
    ...
    def _setup(self):
        ...
        r = hp.add_run("9/18  AI·반도체 시장 코멘트  ·  BOJ + 이은택 + 윤지호  ·  강의노트")
        ...
        r = fp.add_run("영상 10편 + 퀵코멘트를 주제별로 재구성  ·  숫자는 공개 자료로 교차검증  ·  ")
        ...
        core.author = "준혁"
        core.subject = "BOJ, 이은택 W바닥, 윤지호 박스, 엔비디아 현금, 토큰 P↓Q↑, 소부장"
...
def build():
    ...
    n.p("2026. 9. 18. 강의노트  ·  FOMC 다음날 + BOJ 인상일  ·  영상 10편 + 퀵코멘트", size=10.5, color=GRAY, align="center", space_after=4)
    n.p("산은 넘었다. 바닥은 W. 박스는 6,000–7,150. 현금이 답을 안 냈다", size=13, bold=True, color=GOLD, align="center", space_after=2)
    n.p("9월 18일  AI·반도체 시장 코멘트", size=22, bold=True, color=NAVY, align="center", space_after=2)
    n.p("이은택 W바닥  ·  윤지호 박스  ·  엔비디아 현금 40%  ·  토큰 P↓Q↑", size=12, bold=True, color=NAVY2, align="center", space_after=8)

    n.callout(
        "한 장 결론",
        [
            "이은택(자막 이윤택은 오역): 25%+ 급락의 바닥은 V보다 W가 많다. 지금은 두 번째 바닥을 만드는 과정이다.",
            "윤지호: 위로도 아래로도 쉽지 않은 박스. 8,000은 아직 안 연다. 상단은 하이퍼스케일러가 토큰으로 현금을 다시 만들어야 열린다.",
            "엔비디아 Q2 마진은 75%인데 영업현금 전환은 40.3%. 매출채권 DSO 60일. 지분평가를 ‘2분기 상당액’으로 쓰면 과장이다.",
            "버블 붕괴는 미 10년 5.0~5.3% 추세 돌파와 인플레 No Way Back의 AND다. 이번 주 5.04 터치는 금요일 4.95로 해제됐다.",
            "정지훈의 프레임은 그대로다. 병목=노드=돈. 연산은 풀렸고 지금은 메모리, 다음 영수증은 전력·후공정이다.",
        ],
        kind="key",
    )
    ...
    n.table(
        ...
        [
            ...
            ["쓰지 말 것", "Ohio/NY 확정, 젠슨 2배=매출 2배, 원/달러 1,587, 5% 터치=붕괴, PER 역수=20%, CDS 7월 급등, 토큰 2,400%, 하나마이크론=삼성 확정 파트너, 이윤택"],
        ],
        col_widths=[3.6, 14.0],
    )
    ...
    n.table(
        ...
        [
            ...
            ["윤지호 (FN 대담)", "박스, 하이퍼스케일러 현금, 엔비디아 75/40, 토큰·속도조절"],
            ["인포맥스 라이브 주간", "FOMC 둘째 날, 7,150 네 번째, P↓Q↑, 삼성 EPS 조건, 송유관"],
            ["황유현 (신한, 주린이구조대)", "기정사실화, 유가 퍼즐, 순환매, 닉스 210만(해석)"],
            ["소부장 좌담", "HBM4 외주, Q사이클 28년, 원전/전지/광통신, 코스닥 시총"],
    ...
    n.table(
        ...
        [
            ...
            ["엔비디아 Q2", "GM 75.0%\nOCF/NI 40.3%", "매출채권 $63.1B · DSO 60일"],
            ["토큰 8월", "볼륨 YoY 28배\n지출 12배", "가중단가 −28%. 방송 2,400%는 부분"],
        ],
    ...
    n.table(
        ...
        [
            ...
            ["윤지호", "박스. 8,000 아님", "하이퍼스케일러 현금·토큰", "마진≠현금, 속도조절=코스트"],
            ["인포맥스", "7,150 네 번째를 본다", "P↓Q↑, 삼성 EPS 조건", "송유관·추석 연휴 변동"],
            ["황유현", "지수 추세는 아직", "순환매 1·2등, 전력", "7,000 위 물량, 대주주 양도"],
            ["소부장 좌담", "Q는 28년. 지금 풀베팅 아님", "후공정·테스트·선제증설", "소재 후행, 전지 마진"],
        ],
    ...
    n.h1("윤지호 — 박스이지 추세가 아니다", num="13.")
    n.image("13_box_range.png")
    n.p("파이낸셜뉴스 대담. 한줄평은 ‘위로도 아래로도 쉽지 않다’. 논트렌딩, 박스, 비추세. 주가 방향 세 갈래(오르냐 내리냐 횡보냐)로 줄이지 말고, 지금 가격을 결정하는 힘을 찾으라고 한다.")
    n.h2("두 소, 두 곰")
    n.bullet("소: AI 과잉투자에 노출된 한국 기업 이익이 매우 좋아졌다. 작년 상법 개정·주주환원 기대가 겹쳐 주가가 날아올랐다.")
    n.bullet("곰 1: 투자 사이클의 한계. 돈이 떨어지고 빌려 쓰게 되면 한계가 온 것 아니냐는 우려.")
    n.bullet("곰 2: 전쟁·포퓰리즘·방만 재정. 정부와 기업이 동시에 돈을 쓰니 금리가 오른다.")
    n.bullet("금리 자체가 나쁜 게 아니다. 빌려서 그 이상을 벌면 문제 없다. 오펜하이머(골드만)가 미국 10년 5%를 넘기면 좋은 데가 별로 없었다는 말을 책에서 남겼다고 전한다. 원문은 따로 못 박지 못했다. 부분.")
    n.h2("PER 19.5의 역수는 20%가 아니다")
    n.bullet("윤지호: S&P 500 PER 19.5~19.7, 5%면 역수가 20%라 주식이 매력 없다고 했다.")
    n.bullet("부분: FactSet 9/11 선행 PER 19.1, 후행 25.9. 19.5는 선행 근처다.")
    n.bullet("오해: 19.5의 역수는 약 5.1%다. 20%가 아니다. Fed 모델로 10년 5%와 선행 이익수익률을 붙이는 논리는 살릴 수 있다. 주식이 이기려면 앞으로 이익이 더 나와야 한다.")
    n.bullet("주식의 본질은 지분을 갖고 있으면 그 회사가 돈을 더 만든다는 기대. 반도체가 싸 보여도 현금의 근원을 쫓으면 하이퍼스케일러가 나온다.")
    n.h2("대응")
    n.bullet("박스는 트레이딩 바이는 되지만 추세 상승을 기대하기는 어렵다.")
    n.bullet("포트에 따라 답이 다르다. 물린 사람은 반등 때 현금을 만들고, 비어 있는 사람은 하단에서 모은다.")
    n.bullet("피터 린치: 아침마다 등락률 상위·하위를 보지 마라. 내 포트와 상관없다.")
    n.bullet("9–10월은 우려가 가격에 붙는 구간. 8,000을 넘는 강세장은 아니라는 평가. 본인은 주식 비중을 줄이고 현금을 늘리는 중이라고 했다.")
    n.callout("핵심 질문", ["AI 투자 사이클을 누르는 힘이 약해지면 산다. 팽팽하면 계좌만 녹는다. 시점은 변수에 변화가 생길 때다."], kind="note")

    n.h1("엔비디아 — 마진 75, 현금은 40", num="14.")
    n.image("14_nvidia_cash.png")
    n.bullet("사실: Q2 FY27(분기 7/26 마감, 8/26 발표) 매출 $96.2B, GAAP 매출총이익률 75.0%, 순이익 $59.7B, 영업현금 $24.1B. 전환율 40.3%.")
    n.bullet("매출채권 $63.1B. DSO 45일→60일. 우량 고객 복수 분기 계약의 결제 연장. 방송의 ‘외상’은 이 숫자다.")
    n.bullet("부분: ‘2분기 이익의 상당액이 지분 투자 평가이익’. Q2 평가이익은 $7.8B로 순이익의 약 13%. 상반기 $23.7B를 한 분기로 말한 듯하다.")
    n.bullet("부분·쓰지 말 것: CDS 프리미엄이 7월부터 크게 올랐다는 말은 공개 시계열로 확인하지 못했다.")
    n.bullet("인포맥스: 엔비디아도 2025년 1–4월 딥시크+관세로 거의 −40% 빠졌다가 전고점을 뚫고 갔다. 이익(주당순이익)이 안 꺾이면 미국 주식은 우상향한 경우가 많다. 방향은 맞다.")
    n.callout("시장이 묻는 것", ["하이퍼스케일러 현금이 데이터센터나 토큰으로 다시 도느냐. 답이 나오면 상단이 열리고, 못 하면 흐르고, 더 나쁜 답이 나오면 빠진다."], kind="bear")

    n.h1("토큰 — P↓여도 Q가 더 늘면 된다", num="15.")
    n.image("15_token_pq.png")
    n.bullet("사실: JPM이 인용한 OpenRouter 8월. 볼륨 MoM +47%, YoY 28배. 지출 +7%, 12배. 볼륨가중 단가 −28%, YoY −56%.")
    n.bullet("윤지호: OpenAI가 토큰을 많이 만들어 생태가 돌아가야 한다. 비용은 내려오고 품질은 올라가야 한다. Astra는 돈이 너무 든다.")
    n.bullet("아모데이·알트만·머스크·허사비스의 속도조절은 ‘추가 투자가 부담’ 시그널. 죄수의 딜레마 때문에 진짜로 늦추기는 어렵다. 해석.")
    n.bullet("인포맥스 민수: P가 떨어져도 Q가 더 늘면 하이퍼스케일러를 걱정 마라. 윤지호는 그 그림의 신호가 아직 약해서 주가가 못 간다고 본다.")
    n.bullet("부분: 방송 ‘올해 8월까지 31%, 전년 동기 2,400%, 초당 1,000억’은 원소스를 못 박지 못했다. 방향으로만 JPM을 쓴다.")
    n.callout("DevDay", ["9/29에 에이전트·API 토큰·기업 배포가 숫자로 나오는지가 가을 확인이다. 현금이 다시 돈다는 증거가 필요한 자리다."], kind="blue")

    n.h1("금요일 주간 — 7,150 네 번째, 순환매", num="16.")
    n.h2("인포맥스 라이브")
    n.bullet("FOMC는 당일보다 둘째 날이 본심. 첫날 메리츠는 ‘물가를 위해 경기를 희생’, NH는 ‘호미로 막았다’. 나스닥 급등은 NH 쪽에 배팅한 결과.")
    n.bullet("워시가 파월과 다른 사람임을 시장이 믿었다. 연준도 하이퍼스케일러 자금 조달이 10년을 올렸다고 인정했다는 전언. 해석이 섞여 있다.")
    n.bullet("코스피 7,150–7,200을 네 번 두드렸다. 오늘은 6,894. 코스닥은 ‘변비’ — 힘은 쓰는데 거래가 소멸된다.")
    n.bullet("부분: 예탁금 100조. 뉴시스 8/28 약 96.7조로 이미 100조 아래. ‘곧 100조’는 지난 숫자다.")
    n.bullet("부분: 현대차 수출 30%. 산업부 8월 자동차 수출액 −29.8%. 현대 수출 대수 −48.9%. 현대 해외판매 −8.5%와 섞지 말 것.")
    n.bullet("유가: 사우디 동서(Petroline) 송유관 펌프장 피격. 수리 4–6주면 10월. 얀부 처리량 방송 700만 b/d vs Kpler 약 550만. 부분.")
    n.bullet("조건: 삼성 주당순이익 추정치가 27년 4분기까지 안 꺾이면 전고점 자리는 열려 있다. 시장이 그 숫자를 안 믿어서 안 간다. 해석.")
    n.h2("황유현")
    n.bullet("금리 인상은 기정사실화(빼다 콤플리). 남은 퍼즐은 유가. 중간선거 직전에 극적으로 끝내려 할 수 있다. 해석.")
    n.bullet("지수는 아직 추세 전환이 아니다. 대형주 대신 전력·전선·에너지, 화장품, 조선 엔진.")
    n.bullet("너무 급하게 7,000을 넘으면 개인 물량과 대주주 양도 이슈가 코스피로 온다. 150조는 공개 집계가 아니다. 부분.")
    n.bullet("닉스 210–214만은 한 번은 연다. 자사주 매입 종료·10월 중하순과 겹칠 수 있다. 해석. 3·4등주는 사지 마라.")
    n.bullet("쓰지 말 것: 외인 7월까지 180조 매도, 삼성 외인 지분 46–47%(평균 49/52). 확인하지 못했다.")

    n.h1("소부장 — HBM이 온양을 먹고, Q는 28년에 온다", num="17.")
    n.bullet("사실: TheElec 9/15. 삼성은 DDR5 모듈·SSD의 증분을 외주하고 천안·온양을 HBM에 쓴다. 확인된 파트너는 한양디지텍(베트남), 드림텍(인도), SFA세미콘(필리핀).")
    n.bullet("부분: 좌담이 하나마이크론을 삼성 DDR5 파트너로 특정. 공개 보도의 이름은 위 셋이다. 하나마이크론은 베트남 OSAT 증설 수혜 후보다.")
    n.bullet("테스트·소켓·OSAT가 7월 말 이후 상대 강도. DI, 엑시콘, ISC, 하나마이크론, 한양디지텍, PSK홀딩스.")
    n.bullet("부품·소재 실적은 삼성 반등보다 대략 6개월 후행. 심텍 등 기판은 2분기부터 추정치를 상회하기 시작했다.")
    n.bullet("손희원: 블랙 약 180만 장 → 베라루빈 약 280만 장. HBM Q와 서버 디램·소캠이 같이 는다. 선제 증설은 고객 요청의 흔적.")
    n.bullet("해석: Q사이클은 28년. 지금부터 그것 때문에 풀베팅할 수는 없다. 장비는 먼저, 소재는 내년 램프업(청주 15x, 평택 P4).")
    n.bullet("원전은 신재생이 대체할 수 없는 베이스로드. 2차전지는 바닥은 찍었으나 가격 주도권이 없다. 셀(SDI)이 소재보다 낫고, 실적 시즌엔 의심.")
    n.bullet("광통신은 파일럿이 아니라 양산 스케줄이 나오는 단계. 엔비디아에 납품하는 기업과 ‘우리도 한다’를 밸류로 가를 것.")
    n.bullet("부분: 알테오젠 시총 18조. 9월 초 약 20.8조. 에코프로 11조는 미확인. 상징은 주성엔지니어링이 4위라는 말.")
    n.callout("물리 영수증", ["효성중공업 변압기, 두산퓨얼셀, 두산 CCL 증설, 현대일렉 엔진, 아마존–Generac. 기업이 거짓말한 거래가 아니다. 주가가 안 가는 이유는 숫자를 안 믿어서다."], kind="key")

    n.h1("고친 것 · 확인 다섯", num="18.")
    n.table(
        ["방송/코멘트", "고친 값", "이유"],
        [
            ...
            ["PER 19.5의 역수=20%", "역수는 약 5.1%", "선행 PER≈19. 후행은 25.9"],
            ["엔비디아 2분기 상당액=지분평가", "Q2 $7.8B(13%), 상반기 $23.7B", "분기를 혼동"],
            ["엔비디아 CDS 7월 급등", "미확인", "쓰지 말 것"],
            ["토큰 2,400% · 초당 1,000억", "OpenRouter 볼륨 YoY 28배", "방송 숫자 원소스가 없음"],
            ["하나마이크론=삼성 DDR5 파트너", "한양디지텍·드림텍·SFA", "베트남 OSAT 후보는 가능"],
            ["현대차 수출 −30%", "산업 수출액 −29.8%\n현대 대수 −48.9%", "해외판매 −8.5%와 별개"],
            ["예탁금 100조", "8/28 약 96.7조", "이미 100조 아래"],
            ["원/달러 1,500→1,320", "종가 1,383.3", "과거 저점과 오늘을 섞음"],
            ["알테오젠 시총 18조", "9월 초 약 20.8조", "대략치"],
        ],
    ...
    n.callout(
        ...
        [
            ...
            "9/29 DevDay에서 에이전트·API 토큰·기업 배포가 숫자로 나오는지. 현금이 다시 도는지.",
            "Ohio·New York이 탐색에서 계약으로 넘어가는지. Indiana는 이미 별축.",
            "3분기, 하이퍼스케일러 현금과 삼성 모듈 외주·후공정 가동률이 추정치를 받쳐 주는지.",
        ],
        kind="key",
    )

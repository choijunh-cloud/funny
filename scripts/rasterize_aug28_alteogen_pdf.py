...
HTML = ROOT / "reports" / "2026-08-28-alteogen-alt-b4-pages.html"
...
def _write_html(pages: list[Path]) -> None:
    ...
    for i, p in enumerate(pages, 1):
        ...
        if i == len(pages) and i > len(TITLES):
            title = "원문"
        articles.append(
...
<head>
  ...
  <title>알테오젠 ALT-B4 · PDF 페이지</title>
  <style>
...
<body>
  <div class="top">
    <div class="top-inner">
      <div class="brand">ALT-B4 BRIEF · PDF 페이지 · <b>8월 28일</b></div>
      <nav>
        {nav}
        <a href="2026-08-28-alteogen-alt-b4.html">HTML</a>
      </nav>
    </div>
  </div>
  <div class="wrap">
    <h1>A4 {len(pages)}쪽 · 여기서 바로 스크롤</h1>
    <p class="muted">PDF 앱이 없어도 됩니다. 위 번호로 이동. 매수·매도 추천 아님.</p>

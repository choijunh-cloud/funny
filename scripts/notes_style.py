...
class Notes:
    ...
    def table(self, headers, rows, col_widths=None, first_col_bold=True, row_fills=None):
        ...
        for r_i, row in enumerate(rows):
            fill = None
            if row_fills and r_i < len(row_fills) and row_fills[r_i]:
                fill = row_fills[r_i]
            elif r_i % 2 == 1:
                fill = ROW_HEX
            else:
    ...
    def image(self, path, width_cm=16.4, caption=None):
        para = self.doc.add_paragraph()
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        para.paragraph_format.space_before = Pt(4)

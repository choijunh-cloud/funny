...
new Chart(document.getElementById("tpChart"), {
  ...
  plugins: [{
    ...
    afterDraw(chart) {
      ...
      const draw = (xVal, color, label) => {
        const x = scales.x.getPixelForValue(xVal);
        ctx.save();
        ctx.strokeStyle = color;
        ctx.setLineDash([5,4]);
        ctx.beginPath(); ctx.moveTo(x, chartArea.top); ctx.lineTo(x, chartArea.bottom); ctx.stroke();
        ctx.fillStyle = color; ctx.font = "11px sans-serif"; ctx.fillText(label, x+4, chartArea.top+12);
        ctx.restore();
      };
      draw(31000, crimson, "31,000");
      draw(42000, gold, "42,000");
    }
  }]
});
